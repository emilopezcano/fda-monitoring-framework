...
cat("--- Scenario 1A simulation for L1, Bootstrap,", n1, "/", n2, "\n")
start0 <- Sys.time()
...
for (i in seq_along(deltas)) {
  ...
  senal_l1std_boot_200_100[[i]] <- unlist(lapply(senal, function(x) x$s))
  potencia_l1std_boot_200_100[i] <- mean(senal_l1std_boot_200_100[[i]])
...
save(
  ...
  file = "results/simulations/l1std_boot_200_100_1A.RData"
)
...
cat(
  "[",
  format(end, "%HH:%MM"),
  "] END simulation Scenario 1A for L1, Bootstrap,", n1, "/", n2, "\n",
