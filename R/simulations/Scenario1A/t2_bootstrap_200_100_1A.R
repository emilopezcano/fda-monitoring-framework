...
senal_t2_boot_200_100 <- vector("list", length(deltas))

## Object for saving simulation result (power)
potencia_t2_boot_200_100 <- numeric(length(deltas))
...
cat("--- Scenario 1A simulation for T2, Bootstrap,", n1, "/", n2, "\n")
start0 <- Sys.time()
...
for (i in seq_along(deltas)) {
  ...
  senal_t2_boot_200_100[[i]] <- unlist(lapply(senal, function(x) x$s))
  potencia_t2_boot_200_100[i] <- mean(senal_t2_boot_200_100[[i]])
  cat("\t", format(Sys.time() - start, digits = 3), "\n")
}

save(
  potencia_t2_boot_200_100,
  senal_t2_boot_200_100,
  file = "results/simulations/t2_boot_200_100_1A.RData"
)
...
cat(
  "[",
  format(end, "%HH:%MM"),
  "] END simulation Scenario 1A for T2, Bootstrap,", n1, "/", n2, "\n",
