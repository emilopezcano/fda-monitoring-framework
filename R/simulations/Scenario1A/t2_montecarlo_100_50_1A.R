...
potencia_t2_montecarlo_100_50 <- numeric(length(deltas))
senal_t2_montecarlo_100_50 <- vector("list", length(deltas))
...
for(i in seq_along(deltas)) 
{
  ...
  for(g in seq_len(mc_chart)) 
  {
    ...
    ##########################################################################
    # 3. Monitoreo Phase II
    ##########################################################################
    
    senal_grafico_t2_montecarlo_100_50 <- logical(K)
    ...
    for(k in seq_len(K)){
      ...
      senal_grafico_t2_montecarlo_100_50[k] <- T2 > UCL
      
    }
    
    senal_delta[[g]] <- senal_grafico_t2_montecarlo_100_50
    
  }
  
  senal_t2_montecarlo_100_50[[i]] <- unlist(senal_delta)
  
  potencia_t2_montecarlo_100_50[i] <- mean(senal_t2_montecarlo_100_50[[i]])
}


save(
  potencia_t2_montecarlo_100_50,
  senal_t2_montecarlo_100_50,
  file = "t2_montecarlo_100_50.RData"
)
