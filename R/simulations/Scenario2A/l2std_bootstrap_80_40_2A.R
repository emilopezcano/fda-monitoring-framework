...
senal_l2std_boot_80_40 <- vector("list", length(etas))

## Object for saving simulation result (power)
potencia_l2std_boot_80_40 <- numeric(length(etas))
...
cat("--- Scenario 2A simulation for L2, Bootstrap,", n1, "/", n2, "\n")
start0 <- Sys.time()
...
for (i in seq_along(etas)) {
  ...
  senal_l2std_boot_80_40[[i]] <- unlist(lapply(senal, function(x) x$s))
  potencia_l2std_boot_80_40[i] <- mean(senal_l2std_boot_80_40[[i]])
...
save(
  potencia_l2std_boot_80_40,
  senal_l2std_boot_80_40,
  file = "results/simulations/l2std_boot_80_40_2A.RData"
)
...
cat(
  "[",
  format(end, "%HH:%MM"),
  "] END simulation Scenario 2A for L2, Bootstrap,", n1, "/", n2, "\n",
