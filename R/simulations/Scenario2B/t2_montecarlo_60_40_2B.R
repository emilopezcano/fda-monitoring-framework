...
potencia_t2_montecarlo_60_40 <- numeric(length(etas))

## Object for saving simulation result (out of control signal)
senal_t2_montecarlo_60_40 <- vector("list", length(etas))
...
for (i in seq_along(etas)) {
  ...
  for (g in seq_len(mc_chart)) {
    ...
    senal_grafico_t2_montecarlo_60_40 <- logical(K)
    ...
    for (k in seq_len(K)) {
      ...
      senal_grafico_t2_montecarlo_60_40[k] <- T2 > UCL
    }

    senal_eta[[g]] <- senal_grafico_t2_montecarlo_60_40
  }

  senal_t2_montecarlo_60_40[[i]] <- unlist(senal_eta)

  potencia_t2_montecarlo_60_40[i] <- mean(senal_t2_montecarlo_60_40[[i]])
...
save(
  potencia_t2_montecarlo_60_40,
  senal_t2_montecarlo_60_40,
  file = "results/simulations/t2_montecarlo_60_40_2B.RData"
)
...
cat(
  "[",
  format(end, "%HH:%MM"),
  "] END simulation Scenario 2B for T2, Montecarlo,", n1, "/", n2, "\n",
