...
potencia_l1std_montecarlo_60_40 <- numeric(length(etas))

## Object for saving simulation result (out of control signal)
senal_l1std_montecarlo_60_40 <- vector("list", length(etas))
...
for (i in seq_along(etas)) {
  ...
  for (g in seq_len(mc_chart)) {
    ...
    senal_grafico_l1std_montecarlo_60_40 <- logical(K)
    ...
    for (k in seq_len(K)) {
      ...
      senal_grafico_l1std_montecarlo_60_40[k] <- L1std > UCL
    }

    senal_eta[[g]] <- senal_grafico_l1std_montecarlo_60_40
  }

  senal_l1std_montecarlo_60_40[[i]] <- unlist(senal_eta)

  potencia_l1std_montecarlo_60_40[i] <- mean(senal_l1std_montecarlo_60_40[[i]])
...
save(
  potencia_l1std_montecarlo_60_40,
  senal_l1std_montecarlo_60_40,
  file = "results/simulations/l1std_montecarlo_60_40_2B.RData"
)
