...
potencia_l2std_perms_40_20 <- numeric(length(etas))

## Object for saving simulation result (out of control signal)
senal_l2std_perms_40_20 <- vector("list", length(etas))
...
cat("--- Scenario 2B simulation for L2, Permutations,", n1, n2, "\n")
...
for (i in seq_along(etas)) {
  ...
  senal_l2std_perms_40_20[[i]] <- unlist(lapply(senal, function(x) x$s))

  potencia_l2std_perms_40_20[i] <- mean(senal_l2std_perms_40_20[[i]])
...
cat(
    "[",
    format(end, "%HH:%MM"),
    "] END simulation Scenario 2B for L2, Permutations,", n1, n2, "\n",
    format(end - start, digits = 3),
    sep = ""
  )

save(
  potencia_l2std_perms_40_20,
  senal_l2std_perms_40_20,
  file = "results/simulations/l2std_perms_40_20_2B.RData"
)
