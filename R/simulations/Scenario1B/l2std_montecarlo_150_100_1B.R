...
potencia_l2std_montecarlo_150_100 <- numeric(length(deltas))

## Object for saving simulation result (out of control signal)
senal_l2std_montecarlo_150_100 <- vector("list", length(deltas))





cat("--- Scenario 1B simulation for L2, Montecarlo,", n1, "/", n2, "\n")
start0 <- Sys.time()
...
for (i in seq_along(deltas)) {
  ...
  for (g in seq_len(mc_chart)) {
    ...
    senal_grafico_l2std_montecarlo_150_100 <- logical(K)
    ...
    for (k in seq_len(K)) {
      ...
      senal_grafico_l2std_montecarlo_150_100[k] <-
        l2std > UCL
    }

    # Control chart result
    senal_delta[[g]] <-
      senal_grafico_l2std_montecarlo_150_100
  }

  # Final result for delta
  senal_l2std_montecarlo_150_100[[i]] <-
    unlist(senal_delta)

  potencia_l2std_montecarlo_150_100[i] <-
    mean(senal_l2std_montecarlo_150_100[[i]])
  cat("\t", format(Sys.time() - start, digits = 3), "\n")
}

## Save ----
save(
  potencia_l2std_montecarlo_150_100,
  senal_l2std_montecarlo_150_100,
  file = "results/simulations/l2std_montecarlo_150_100_1B.RData"
)
...
cat(
  "[",
  format(end, "%HH:%MM"),
  "] END simulation Scenario 1B for L2, Montecarlo,", n1, "/", n2, "\n",
