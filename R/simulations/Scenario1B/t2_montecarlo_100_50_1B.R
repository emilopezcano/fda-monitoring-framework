...
for (i in seq_along(deltas)) {
  ...
  for (g in seq_len(mc_chart)) {
    ...
    for (k in seq_len(K)) {
      ...
      senal_grafico_t2_montecarlo_100_50[k] <-
        T2 > UCL
    }

    # Control chart result
    senal_delta[[g]] <-
      senal_grafico_t2_montecarlo_100_50
  }

  # Final result for delta
  senal_t2_montecarlo_100_50[[i]] <-
    unlist(senal_delta)

  potencia_t2_montecarlo_100_50[i] <-
    mean(senal_t2_montecarlo_100_50[[i]])
...
save(
  ...
  file = "results/simulations/t2_montecarlo_100_50_1B.RData"
)
