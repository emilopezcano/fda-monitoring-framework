...
potencia_l1std_perms_100_50 <- numeric(length(deltas))

## Object for saving simulation result (out of control signal)
senal_l1std_perms_100_50 <- vector("list", length(deltas))
...
cat("--- Scenario 1B simulation for L1, Permutations,", n1, "/", n2, "\n")
start0 <- Sys.time()
...
for (i in seq_along(deltas)) {
  ...
  senal_l1std_perms_100_50[[i]] <- unlist(lapply(senal, function(x) x$s))

  potencia_l1std_perms_100_50[i] <- mean(senal_l1std_perms_100_50[[i]])
...
save(
  ...
  file = "results/simulations/l1std_perms_100_50_1B.RData"
)
...
cat(
  "[",
  format(end, "%HH:%MM"),
  "] END simulation Scenario 1B for L1, Permutations,", n1, "/", n2, "\n",
