...
potencia_l2std_perms_150_100 <- numeric(length(deltas))

## Object for saving simulation result (out of control signal)
senal_l2std_perms_150_100 <- vector("list", length(deltas))
...
cat("--- Scenario 1B simulation for L2, Permutations,", n1, "/", n2, "\n")
start0 <- Sys.time()
...
for (i in seq_along(deltas)) {
  ...
  senal_l2std_perms_150_100[[i]] <- unlist(lapply(senal, function(x) x$s))

  potencia_l2std_perms_150_100[i] <- mean(senal_l2std_perms_150_100[[i]])
  cat("\t", format(Sys.time() - start, digits = 3), "\n")
}

save(
  potencia_l2std_perms_150_100,
  senal_l2std_perms_150_100,
  file = "results/simulations/l2std_perms_150_100_1B.RData"
)
...
cat(
  "[",
  format(end, "%HH:%MM"),
  "] END simulation Scenario 1B for L2, Permutations,", n1, "/", n2, "\n",
