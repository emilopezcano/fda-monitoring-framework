...
potencia_l2std_perms_100_50 <- numeric(length(deltas))

## Object for saving simulation result (out of control signal)
senal_l2std_perms_100_50 <- vector("list", length(deltas))
...
for (i in seq_along(deltas)) {
  ...
  senal_l2std_perms_100_50[[i]] <- unlist(lapply(senal, function(x) x$s))

  potencia_l2std_perms_100_50[i] <- mean(senal_l2std_perms_100_50[[i]])
  cat("\t", format(Sys.time() - start, digits = 3), "\n")
}

save(
  potencia_l2std_perms_100_50,
  senal_l2std_perms_100_50,
  file = "results/simulations/l2std_perms_100_50_1B.RData"
)
